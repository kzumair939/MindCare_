...
public class SessionRestController {
    ...
    private boolean isSessionParticipantOrAdmin(Session session, Authentication auth) {
        ...
        boolean isUser = session.getUser() != null &&
                (username.equals(session.getUser().getUsername()) || username.equals(session.getUser().getEmail()));

        boolean isTherapist = session.getTherapist() != null && session.getTherapist().getUserAccount() != null &&
                (username.equals(session.getTherapist().getUserAccount().getUsername())
                        || username.equals(session.getTherapist().getUserAccount().getEmail()));

        return isUser || isTherapist;
    }
    ...
    public ResponseEntity<?> getMessages(@PathVariable Long id, Authentication auth) {
        ...
        if (!isSessionParticipantOrAdmin(s, auth)) {
            return ResponseEntity.status(403)
                    .body(Map.of("error", "You are not authorized to view messages for this session"));
        }
        ...
        List<Map<String, Object>> result = msgs.stream().map(m -> {
            ...
            boolean mine = m.getSenderName().equals("You") ||
                    ...
                    (s.getTherapist() != null && s.getTherapist().getUserAccount() != null
                            && m.getSenderName().equals(s.getTherapist().getUserAccount().getUsername()));
            map.put("mine", mine);
            return map;
    ...
    public ResponseEntity<?> bookSession(@RequestBody Map<String, Object> req, Authentication auth) {
        try {
            ...
            SessionResponseDTO dto = sessionService.bookSession(auth.getName(), therapistId, date, time, mode, type,
                    null, false);
    ...
    public ResponseEntity<?> onlineInfo(@PathVariable Long id, Authentication auth) {
        ...
        if (!isSessionParticipantOrAdmin(s, auth)) {
            return ResponseEntity.status(403).body(Map.of("error", "You are not authorized to view this session"));
        }
    ...
    public ResponseEntity<?> updateSessionStatus(@PathVariable Long id, @RequestBody Map<String, Object> body,
            Authentication auth) {
